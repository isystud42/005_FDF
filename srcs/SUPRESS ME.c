...
int		trace_l_to_r(t_fdf_env *env, int *xytab, int clrxy, int clrxY1)
{
	...
	save = X2;
	while (save++ < X1)
		PXL(MLX,WIN,save,Y1 + ((Y2 - Y1) * (save - X1)) / (X2 - X1), ft_handle_pixel_color(clrxy,clrxY1,100*(X1 - save)/(X1 - X2)));
	save = Y2;
	while (save++ < Y1)
		PXL(MLX, WIN, X1 + ((X2 - X1) *	(save - Y1)) / (Y2 - Y1), save, ft_handle_pixel_color(clrxy,clrxY1,100*(Y1 - save)/(Y1 - Y2)));
	save = Y1;
	while (save++ < Y2)
		PXL(MLX, WIN, X1 + ((X2 - X1) *	(save - Y1)) / (Y2 - Y1), save, ft_handle_pixel_color(clrxy,clrxY1,100*(Y2 - save)/(Y2 - Y1)));
	return (0);
}
...
int		trace_r_to_l(t_fdf_env *env, int *xytab, int clrxy, int clrxY1)
{
	...
	save = Y1;
	while (X1 == X2 && Y1++ < Y2)
		PXL(MLX,WIN,X1,Y1,ft_handle_pixel_color(clrxy,clrxY1,100 * (Y2 - Y1)/(Y2 - save)));
	Y1 = save;
	save = X1;
	while (X1++ < X2 && (Y1) == Y2)
		PXL(MLX,WIN,X1,Y1,ft_handle_pixel_color(clrxy,clrxY1,100 * (X2 - X1)/(X2 - save)));
	X1 = save;
	trace_r_to_l2(env,xytab, CLR[I + 1][J], CLR[I][J]);
	return (0);
}
